...
hotelController.SearchHotels = async (req, res) => {
    try {
        ...
        if (req.body.searchQuery) {
            const {searchQuery, sync, destinationCountryCode} = req.body;
            const {checkinDate, checkoutDate, roomInfo, searchCriteria, searchPreferences} = searchQuery;
            ...
            const {cityName, cityId, checkIn, checkOut, guests, LocationID, rooms, destinationCountryCode} = req.body;
        ...
        console.log("---")
        // console.log(mergedHotelData.data.hotels)
        const hotelRates = mergedHotelData.data.hotels.map(hotel => hotel.rate?.Rateinfo?.totalRate || "N/A");
        ...
        for (const hotel of mergedHotelData.data.hotels) {
            ...
            for (const markup of agentMarkupData) {
                ...
                // else if (markup['trip_type'] === "Domestic") {
                //     if (markup['markup_type'] === "Fixed") {
                //         markupAmount = parseFloat(markup['markup_amount']) || 0;
