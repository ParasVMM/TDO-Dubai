...
flightController.AirSell = async(req,res) => {
    ...
    let apiUrl = `https://api.air.prod.trvlnxt.com/api/AirSell`;
    ...
    response = await axios.post(apiUrl, payload,{
           headers: {
               'ApiKey': `CUST001741GoldenGatewaysGEITlZTJUrVe1dMnJMuTMJPMcI7ObfYhYEjaSg2`,
...
flightController.ticket_old = async(req,res) => {
    ...
    try
    {
        let apiUrl = `https://api.air.prod.trvlnxt.com/api/Booking`;
...
flightController.ticket = async (req, res) => {
    ...
    const apiUrl = 'https://api.air.prod.trvlnxt.com/api/Booking';
    try {
...
flightController.flightResultsApi = async (req, res) => {
    ...
    try {
        ...
        let apiUrl = `https://api.air.prod.trvlnxt.com/api/Availability`;
...
flightController.getAllFlights = async (req, res) => {
    try {
        ...
        let { from, to, travelDate, fare_type, adults, child, infants, ResultFareType } = req.body;
                ...
                console.log(tboResponse.value.data.ResponseStatusType)
                        ...
                        let groupedFlights = tboResponse.value.data.Journeys[0].Flights.reduce((acc, flight) => {
                            ...
                            let segments = mySegments.map(segment => {
                                ...
                                let obj = {
                                    ...
                                    //Baggage: segment?.Baggages || Array.isArray(segment.Baggage) ? segment.Baggage : [segment.Baggage],
    ...
    if (tax?.TaxCode === 'K3') {
        ...
        otherTaxesSum += tax?.TaxAmount;
                            ...
                            let fareBreakup = {
                                ...
                                PublishedFare: parseFloat(flight.GrossFare),
                            ...
                            let Baggage = {
                                Baggage: `${mySegments?.[0]?.Baggage?.Weight || '15'}-${mySegments?.[0]?.Baggage?.Unit || 'KG'}`,
                                CabinBaggage: mySegments?.[0]?.CabinBaggage || '7Kg'
                            }
...
flightController.release_booking = async(req,res) => {
    ...
    let transaction ;
    let {id} = req.body;
    const url = 'https://api.air.prod.trvlnxt.com/api/CancellationBooking';
    try
    {
        transaction = await connectToDatabase();
...
flightController.confirm_hold = async(req,res) => {
    ...
    const url = 'https://api.air.prod.trvlnxt.com/api/HoldBooking';
    try
    {
        transaction = await connectToDatabase();
