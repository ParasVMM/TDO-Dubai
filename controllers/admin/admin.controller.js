...
adminController.updateGuestDetails = async (req, res) => {
    ...
    try {
        ...
        if (pt === 'Master') {
            ...
            console.log("Guest Count:", data.length);

            if (data.length > 1) {
                // Update adult details (don't block updates, just prevent deletions)
                let [result] = await adminService.UpdateGuestInfo(connection, booking_id, pt,lN,fN );
                return res.send({
                    responseCode: 2,
                    error: false,
                    warning: false,
                ...
                console.log("Only one adult left");
            }
        }

        // Update Child/Other guest details
        ...
        if (connection) connection.release();  // Return connection to the pool
    }
};
