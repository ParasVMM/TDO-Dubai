let showChart3 = (charType) => {
    // alert(charType)
    document.getElementById("activityz3").innerHTML = ""
    /* "use strict" */
    if (charType === "weekly") {
        let formData = new FormData();
        formData.append("agentEmail", localStorage.getItem("agentEmail"))
        formData.append("type","weekly")
        fetch("/fetchHotelChartCount", {
            method: "post",
            body: formData
...
console.log(sum);
            ...
            console.log(amtArr)
            document.getElementById("hotelTotalAmt").innerText = sum.toFixed(2) + " (Weekly)"
            var dzChartlist = function () {
                ...
                var activityz = function () {
                    ...
                    var chartArea = new ApexCharts(document.querySelector("#activityz3"), optionsArea);
                    chartArea.render();
        ...
        fetch("/fetchHotelChartCount",{
            ...
            // console.log(amtArr)
            document.getElementById("hotelTotalAmt").innerText = sum.toFixed(2) + " (Monthly)"
            var dzChartlist = function () {
                var screenWidth = $(window).width();
                let draw = Chart.controllers.line.__super__.draw; //draw shadow
                var activityz = function () {
        ...
        fetch("/fetchHotelChartCount",{
            ...
            // console.log(amtArr)
            document.getElementById("hotelTotalAmt").innerText = sum.toFixed(2) + " (Yearly)"
            var dzChartlist = function () {
                var screenWidth = $(window).width();
                let draw = Chart.controllers.line.__super__.draw; //draw shadow
                var activityz = function () {
