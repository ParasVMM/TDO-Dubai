...
flightsServices.AgentMarkup = async (connection, agentEmail) => {
    return connection.execute(`  SELECT *
            FROM travelagent_markup_flight
            WHERE created_by = ? AND plan_type = 'Flight';`, [agentEmail]);
}

flightsServices.TdoMarkup = async (connection) => {
    return connection.execute(`  SELECT *
            FROM platform_fee
            WHERE product = 'Flights'`);
}
